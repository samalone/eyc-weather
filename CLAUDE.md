...
> **Status:** Integrated. `src/wunderground.js` fetches/caches the current observation; `src/conditions.js` orchestrates the `/api/conditions` card — it **favors WU** for local temp/wind/humidity/dew point/pressure/precip, borrows the sky condition & description from NWS (WU has none), and **falls back entirely to NWS** when the WU station is offline, stale (>30 min), or the key is missing. The wind compass also plots WU as a second source: `getWuWind()` serves the WU station's last hour of rapid observations (`observations/all/1day`, filtered to 60 min) via `/api/observations/wunderground/wind`. (Note: the KRICRANS68 wind vane has a friction set-screw that slips, so its **direction is unreliable** — by default the PWS is shown **speed-only**: omitted from the compass and listed without a heading in the legend, via the `speedOnly` flag in `WIND_SOURCES` (`public/index.html`). Its speed is trustworthy. The real fix is mechanical — tighten the set-screw — but between slips the error is a roughly **constant rotational offset**, so the app can compensate: the unlisted **`/recalibrate`** page estimates the offset by comparing the PWS against NOAA Providence over the last hour (per-sample circular mean of the direction differences, gated on wind ≥ 5 kt; see `computeRecalibration` in `wunderground.js`) and stores it in `calibration.offsetDeg` (the runtime successor to the old `WIND_DIR_OFFSET_DEG` constant). The offset is **in-memory only** — a pod restart reverts to uncalibrated (offset 0, speed-only). When `getCalibration().active` is true, the frontend (`wuDirCalibrated`) restores the PWS direction trail and legend heading. Apply/reset are gated by `RECALIBRATE_SECRET`; preview and state reads are public.)
...
| `RECALIBRATE_SECRET` | No | Shared secret gating the `/recalibrate` page's apply/reset actions. If unset, recalibration is disabled (503). |
